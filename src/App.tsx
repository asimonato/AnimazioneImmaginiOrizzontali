...
function wrapN(v: number, p: number) {
  return ((v % p) + p) % p;
}
...
function stripStarts(imgs: ImgDef[], gap: number) {
  ...
  for (let i = 1; i < imgs.length; i++)
    out.push(out[i - 1] + imgs[i - 1].w + gap);
  return out;
}
...
function computeLayout(scale: number): Layout {
  ...
  // Per-pair variable gaps: ensures left-img.leftEdge=0 and right-img.rightEdge=viewportW at each snap
  const n2 = BASE_R2W.length;
  const gap2Arr = BASE_R2W.map(
    (w, i) => (BASE_W - w - BASE_R2W[(i + 1) % n2]) * scale,
  );

  // Strip starts with variable gaps
  const r2starts = Array.from({ length: n2 }, (_, i) => {
    let s = 0;
    for (let j = 0; j < i; j++) s += r2[j].w + gap2Arr[j];
    return s;
  });
  const P2 = r2starts[n2 - 1] + r2[n2 - 1].w + gap2Arr[n2 - 1];

  // Snap: o2 = r2starts[i] → left edge of image i at x=0, right edge of image i+1 at viewportW
  const SNAPS2 = r2starts.map((s) => wrapN(s, P2));

  // Mask per snap: covers the gap between the two visible images
  const mask2L = r2.map((img) => img.w); // left = right edge of left image
  const mask2W = gap2Arr; // width = gap between images
...
// ─── StripR2 — strip with variable gaps ───────────────────────────────────────
function StripR2({
  imgs,
  gaps,
  imgH,
  ...
  const period = imgs.reduce((s, img, i) => s + img.w + gaps[i], 0);
  const reps =
    period > 0
      ? Math.min(30, Math.max(3, Math.ceil((BASE_W * 2 + period) / period) + 1))
      : 3;
        ...
        >
          ...
          >
            <svg
              width={px(21.9054)}
              height={px(24.5749)}
              viewBox="0 0 21.9054 24.5749"
              fill="none"
            >
              <path
                d="M0 12.2875H21.4054"
                stroke="#575756"
                strokeLinejoin="round"
              />
              <path
                d={svgPaths.p3734a680}
                stroke="#575756"
                strokeLinejoin="round"
              />
            </svg>
...
export default function App() {
  const [scale, setScale] = useState(() =>
    safeScale(window.innerWidth / BASE_W),
  );
  const layoutRef = useRef<Layout>(
    computeLayout(safeScale(window.innerWidth / BASE_W)),
  );
  ...
  const mask2El = useRef<HTMLDivElement>(null);
  const snapIdx2 = useRef(0);
  ...
  useEffect(() => {
    ...
    const apply = () => {
      if (r1El.current)
        r1El.current.style.transform = `translateX(${-o1.current}px)`;
      if (r2El.current)
        r2El.current.style.transform = `translateX(${-o2.current}px)`;
      if (r3El.current)
        r3El.current.style.transform = `translateX(${-o3.current}px)`;
      if (mask2El.current) {
        const L = layoutRef.current;
        const idx = snapIdx2.current;
        mask2El.current.style.left = `${L.mask2L[idx]}px`;
        mask2El.current.style.width = `${L.mask2W[idx]}px`;
      }
    };
    ...
    const snapStep = (
      o: React.MutableRefObject<number>,
      tgt: number,
      p: number,
    ) => {
    ...
    const engageSnap = () => {
      ...
      const s2Idx = L.SNAPS2.findIndex((s) => s === snap2.current);
      snapIdx2.current = s2Idx >= 0 ? s2Idx : 0;
      vel.current = 0;
    ...
    const onScroll = () => {
      ...
      vel.current = Math.max(
        -maxVel,
        Math.min(maxVel, vel.current + delta * SCROLL_FACTOR * L.scale),
      );
      if (!raf.current) raf.current = requestAnimationFrame(tick);
      if (idleTimer.current) clearTimeout(idleTimer.current);
      idleTimer.current = setTimeout(engageSnap, WHEEL_IDLE_MS);
    ...
    const onResize = () => {
      ...
      if (isSnapping.current) {
        ...
        const s2Idx = newLayout.SNAPS2.findIndex((s) => s === snap2.current);
        snapIdx2.current = s2Idx >= 0 ? s2Idx : 0;
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <StripR2 imgs={L.r2} gaps={L.gap2Arr} imgH={L.imgH} />
        </div>
      </div>
      ...
      <div
        ...
        style={{
          position: "absolute",
          top: L.row2Y,
          left: L.mask2L[snapIdx2.current],
          width: L.mask2W[snapIdx2.current],
          height: L.imgH,
          backgroundColor: "white",
          zIndex: 1,
        }}
